/* =========================================================
   СНК · sncard.ru — общий слой анимаций (все страницы).
   GSAP 3 + ScrollTrigger + CustomEase, Lenis. Перенесён из
   ../snc-service/js/motion.js (та же семья): единый rAF-тикер,
...
(() => {
  ...
  function inlineIcons(scope = document) {
    $$('.card__icon, .tile__ic', scope).forEach(svg => {
      const use = $('use', svg);
  ...
  /* Плитки категорий оборудования: каскад, spotlight-рамка, наклон ≤ 4°, иконка оживает */
  function tileCards(scope = document) {
    const tiles = $$('.tiles .tile, .bento--cat .card', scope);
    if (!tiles.length) return;
    G.set(tiles, { opacity: 0 });
    ST.batch(tiles, {
      start: 'top 90%', once: true,
      onEnter: batch => G.fromTo(batch, { opacity: 0, scale: 0.96, y: 14 }, { opacity: 1, scale: 1, y: 0, duration: 0.7, stagger: 0.05, ease: EASE, clearProps: 'opacity,y,scale' }),
    });
    tiles.forEach(tile => {
      const on = () => tile.classList.add('is-live');
      const off = () => tile.classList.remove('is-live');
      ...
      const rx = G.quickTo(tile, 'rotationX', { duration: 0.5, ease: 'power3.out' });
      const ry = G.quickTo(tile, 'rotationY', { duration: 0.5, ease: 'power3.out' });
      G.set(tile, { transformPerspective: 800 });
      tile.addEventListener('pointermove', e => {
        const r = tile.getBoundingClientRect();
        const px = (e.clientX - r.left) / r.width, py = (e.clientY - r.top) / r.height;
        const spot = $('.card__spot', tile);
        ...
        rx((0.5 - py) * 8); ry((px - 0.5) * 8); // максимум ±4°
      });
  ...
  window.SNCFX = {
    ...
    inlineIcons, tileCards, ctaReveal, officeReveal,
  };
})();
