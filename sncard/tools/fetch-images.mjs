...
const man = JSON.parse(fs.readFileSync(path.join(ROOT, 'data/images.json'), 'utf8'));
...
for (const [remote, dest] of Object.entries(man)) {
  ...
  try {
    const res = await fetch(encodeURI(url), { headers: { 'user-agent': 'Mozilla/5.0', referer: SITE + '/' }, signal: AbortSignal.timeout(30000) });
...
if (ok) console.log('Дальше: node sncard/tools/generate.mjs — страницы подхватят картинки.');
