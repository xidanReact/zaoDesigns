...
(() => {
  ...
  /* =========================================================
     В корзину: пока только уведомление о недоступности
     ========================================================= */
  // Заказ через сайт пока не работает: в корзину не добавляем, показываем уведомление.
  // Прежняя логика (полёт миниатюры, S.cart.add, статус под кнопкой) — в истории git
  $$('[data-add]').forEach(btn => btn.addEventListener('click', () => {
    S.toast('Заказ через сайт временно недоступен. Позвоните нам или напишите — поможем оформить.');
  }));
